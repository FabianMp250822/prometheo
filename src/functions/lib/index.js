 ...
 * collection.
 */
export const onNewPaymentCreate = onDocumentCreated("pensionados/{pensionadoId}/pagos/{pagoId}", async (event) => {
    var _a;
    ...
    if (!paymentData || !Array.isArray(paymentData.detalles)) {
        logger.info(`Payment ${pagoId} has no data or "detalles" field. Exiting.`);
        return;
    }
    // Filter for details that match our sentence concepts.
    const sentenceConcepts = paymentData.detalles.filter((detail) => SENTENCE_CONCEPT_PREFIXES.some((prefix) => { var _a; return (_a = detail.nombre) === null || _a === void 0 ? void 0 : _a.startsWith(prefix); }));
    ...
    logger.info(`Found ${sentenceConcepts.length} concepts in pmt ${pagoId}.`);
    const newProcessDocRef = db.collection("procesoscancelados").doc();
    const fechaLiquidacionDate = ((_a = paymentData.fechaProcesado) === null || _a === void 0 ? void 0 : _a.toDate) ?
        paymentData.fechaProcesado.toDate() : new Date();
    const newProcessData = {
        año: paymentData.año,
        conceptos: sentenceConcepts.map((c) => {
            var _a;
            return ({
                codigo: c.codigo || ((_a = c.nombre) === null || _a === void 0 ? void 0 : _a.split("-")[0]) || "",
                nombre: c.nombre,
    ...
    try {
        ...
        logger.info(`Created proc doc ${newProcessDocRef.id} for pmt ${pagoId}.`);
    }
    catch (error) {
...
// ===============================================
export const sendPaymentReminder = onCall({ cors: ALLOWED_ORIGINS }, // Firebase handles CORS for callable functions
async (request) => {
    ...
    const { emailUsuario, nombreUsuario, deudaActual, cuotaSugerida, } = request.data;
    if (!emailUsuario || !nombreUsuario || !deudaActual || !cuotaSugerida) {
        logger.error("Missing parameters in the request", request.data);
        throw new HttpsError("invalid-argument", "Required parameters: emailUsuario, nombreUsuario, deudaActual, cuotaSugerida.");
    }
...
<body>
  <div class="container">
    ...
    try {
        ...
        throw new HttpsError("internal", "Error sending reminder email", error.message);
    }
});
// =====================================
...
export const triggerProviredSync = onCall({ cors: ALLOWED_ORIGINS }, async (request) => {
    ...
    try {
        ...
        const newNotifications = (await makeApiRequest("notification", "getData"));
        ...
        for (let i = 0; i < notificationsToSave.length; i += BATCH_SIZE) {
            ...
            chunk.forEach((item) => {
                ...
                const dataToSet = Object.assign(Object.assign({}, item), { uniqueId: uniqueId, demandante_lower: (item.demandante || "").toLowerCase(), demandado_lower: (item.demandado || "").toLowerCase(), syncedAt: Timestamp.now() });
        ...
        throw new HttpsError("internal", "Error durante la sincronización", error.message);
    }
});
// =====================================
...
export const createUser = onCall({ cors: ALLOWED_ORIGINS }, async (request) => {
    ...
    if (((_a = callerUserRecord.customClaims) === null || _a === void 0 ? void 0 : _a.role) !== "Administrador") {
...
export const listUsers = onCall({ cors: ALLOWED_ORIGINS }, async (request) => {
    ...
    try {
        ...
        const users = userDocs.docs.map((doc) => (Object.assign({ id: doc.id }, doc.data())));
        return users;
    }
    catch (error) {
...
export const submitPublicForm = onCall({ cors: ALLOWED_ORIGINS }, async (request) => {
    ...
    try {
        if (formType === "newsletter") {
            if (!data.email)
                throw new HttpsError("invalid-argument", "Newsletter form requires an email.");
