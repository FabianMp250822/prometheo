...
async function fetchExternalData(url: string): Promise<any> {
    try {
        ...
        if (!response.ok) {
            // Provide specific feedback for common HTTP errors
            if (response.status === 403) {
                 throw new Error("Error de red: 403 Forbidden. El servidor externo está bloqueando la solicitud. Esto puede deberse a medidas de seguridad como un firewall. Contacte al administrador del servidor externo.");
            }
             if (response.status === 404) {
                 throw new Error(`Error de red: 404 Not Found. El recurso en la URL ${url} no fue encontrado.`);
            }
        ...
        try {
            ...
            throw new Error(`La respuesta del servidor externo no es un JSON válido. Contenido: ${textResponse.substring(0, 200)}...`);
        }
    } catch (error: any) {
        logger.error(`Error fetching from ${url}:`, error.message);
        // Rethrow a user-friendly error to be caught by the calling function
