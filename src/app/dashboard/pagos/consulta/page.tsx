...
export default function PagosPage() {
    ...
    const fetchInitialPensioners = useCallback(async (isInitial = true) => {
        ...
        try {
            ...
            let newPensioners = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Pensioner));
            const lastVisible = querySnapshot.docs[querySnapshot.docs.length - 1];
            
            setLastDoc(lastVisible || null);
            setHasMore(querySnapshot.docs.length === ITEMS_PER_PAGE);

            if (selectedPensioner && isInitial) {
                const foundIndex = newPensioners.findIndex(p => p.id === selectedPensioner.id);
                if (foundIndex > -1) {
                    const [pensioner] = newPensioners.splice(foundIndex, 1);
                    newPensioners.unshift(pensioner);
                } else {
                     newPensioners.unshift(selectedPensioner);
                }
            }
    ...
    }, [buildQuery, lastDoc, selectedPensioner]);
    ...
    }, [debouncedSearchTerm, buildQuery]);
